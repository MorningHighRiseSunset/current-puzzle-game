...
function renderBoardState(pieces) {
    // Ensure pieces is an array or convert it if it's not
    const piecesArray = Array.isArray(pieces) ? pieces : Object.values(pieces);
    
    // Only remove staged pieces, not played pieces
    const stagedPieces = document.querySelectorAll('.staged-piece');
    stagedPieces.forEach(piece => piece.remove());
    
    // Iterate through the pieces array and add new pieces
    piecesArray.forEach(piece => {
        if (piece && piece.pos) {  // Changed to check piece.pos instead of x/y
            const pieceElement = document.createElement('piece');
            pieceElement.classList.add('played-piece');  // Add played-piece class
            pieceElement.classList.add('small-piece');
            pieceElement.dataset.letter = piece.letter;
            pieceElement.dataset.score = piece.score;
            pieceElement.dataset.coords = JSON.stringify({x: piece.pos.x, y: piece.pos.y});
            
            pieceElement.innerHTML = `${piece.letter}<score>${piece.score}</score>`;
            
            document.querySelector('#game-board').appendChild(pieceElement);
            placePieceOnBoard(pieceElement, piece.pos.x, piece.pos.y);
        }
    });
}
